...
```console
kubectl get secret vault-keys -o json | jq '.data.keys' -r | base64 -d | jq .root_token -r
```

Get vault port

```console
kubectl get svc -l "app=vault,release=vault" -o json | \
...
```console
kubectl get pods -l "app=vault,component=server" -o json | \
  jq -r '.items[] | select(.status.phase | contains("Running")) | .metadata.name' | \
  awk '{print "kubectl exec "$1" vault -- status -address http://127.0.0.1:8200 -format json"}' | sh | jq .sealed
```
